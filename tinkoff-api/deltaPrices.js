...
const deltaMorningCurrentPrice = async () => {
  ...
  for (let figi of figis) {
    ...
    morningPrice =
      morningPrice.candles.length == 0
        ? "Нет данных по открытию"
        : morningPrice.candles[0].c;
    ...
    const delta =
      typeof morningPrice == "number"
        ? currentPrice - morningPrice
        : morningPrice;
...
const portfolioDelta = async () => {
  ...
  data.forEach((operation) => {
    operation.currency === "USD"
      ? incomesUSD.push(operation.payment)
      : incomesRUB.push(operation.payment);
  });
...
const portfolioState = async () => {
  ...
  sum.rub_cur = getCurrency.currencies[1].balance;
  sum.usd_cur = getCurrency.currencies[2].balance;
  sum.currentUSD = await currentPriceGetter(FIGIUSD);
  sum.totalRUB =
    sum.rub +
    sum.rub_cur +
    (sum.usd + sum.usd_cur) * (await currentPriceGetter(FIGIUSD));
