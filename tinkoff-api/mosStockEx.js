...
const getExchangeUSD = async (date) => {
  let exchRub = await fetch(
    "https://iss.moex.com/iss/engines/currency/markets/selt/securities/USD000000TOD.json?iss.meta=off"
  );
...
const getBondPrice = async (index, URL, bondInfo) => {
  ...
  if (bondInfo.ok && bondCandle.ok) {
    ...
    bondPrice =
      ...
      couponValue * securitiesMongo[index].quantity;
    return bondPrice;
  } else {
  ...
  for (let i in securitiesMongo) {
    ...
    if (securityType === "stock") {
      ...
      if (response.ok) {
        ...
        sum.totalRUB =
          sum.totalRUB + securitiesPrice * securitiesMongo[i].quantity;
      } else {
        console.log("Ошибка HTTP: " + response.status);
      }
      ...
      URL = `https://iss.moex.com/iss/engines/stock/markets/bonds/securities/${security}/candles.json?from=${date}`;
      let bondInfo =
        "https://iss.moex.com/iss/engines/stock/markets/bonds/boards/TQCB/securities.json?iss.meta=off&iss.only=securities";
      ...
      URL = `https://iss.moex.com/iss/engines/stock/markets/bonds/securities/${security}/candles.json?from=${date}`;
      let bondInfo =
        "http://iss.moex.com/iss/engines/stock/markets/bonds/boards/TQOD/securities.json?iss.meta=off";
      bondPrice = await getBondPrice(i, URL, bondInfo, securityCurrency);
      ...
      sum.totalRUB =
        sum.totalRUB + securitiesMongo[i].quantity * sum.currentUSD;
